...
const isImmutable = obj => Immutable.Iterable.isIterable(obj);
...
function verifyStateShape(initialState, currentState) {
  const reducerKeys = currentState.keySeq();
...
export default function combineReducers(reducers) {
  ...
  finalReducers.forEach((reducer, key) => {
    ...
    const type = Math.random().toString(36).substring(7).split('')
      .join('.');
  ...
  const defaultState = finalReducers.map(r => undefined);
